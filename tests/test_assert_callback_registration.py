...
class MockAutomation(hass.Hass):
    ...
    def initialize(self):
        if self.should_listen_state:
            self.listen_state(
                self._my_listen_state_callback, "some_entity", new="off"
            )
        ...
        if self.should_register_run_daily:
            self.run_daily(
                self._my_run_daily_callback,
                time(hour=3, minute=7),
                extra_param="ok",
            )
        if self.should_register_run_minutely:
            self.run_minutely(
                self._my_run_minutely_callback,
                time(hour=3, minute=7),
                extra_param="ok",
            )
        if self.should_register_run_at:
            self.run_at(
                self._my_run_at_callback,
                datetime(2019, 11, 5, 22, 43, 0, 0),
                extra_param="ok",
            )
...
class TestAssertListenState:
    def test_success(self, automation: MockAutomation, assert_that):
        ...
        assert_that(automation).listens_to.state(
            "some_entity", new="off"
        ).with_callback(automation._my_listen_state_callback)

    def test_failure__not_listening(
        self, automation: MockAutomation, assert_that
    ):
    ...
    def test_failure__wrong_entity(
        self, automation: MockAutomation, assert_that
    ):
        automation.enable_listen_state_during_initialize()
    ...
    def test_failure__wrong_kwargs(
        self, automation: MockAutomation, assert_that
    ):
        automation.enable_listen_state_during_initialize()
        ...
        with pytest.raises(AssertionError):
            assert_that(automation).listens_to.state(
                "some_entity", new="WRONG"
            ).with_callback(automation._my_listen_state_callback)

        with pytest.raises(AssertionError):
            assert_that(automation).listens_to.state(
                "some_entity", wrong="off"
            ).with_callback(automation._my_listen_state_callback)

    def test_failure__wrong_callback(
        self, automation: MockAutomation, assert_that
    ):
        automation.enable_listen_state_during_initialize()
...
class TestAssertListenEvent:
    def test_success(self, automation: MockAutomation, assert_that):
        ...
        assert_that(automation).listens_to.event(
            "zwave.scene_activated", scene_id=3
    ...
    def test_failure__wrong_kwargs(
        ...
        with pytest.raises(AssertionError):
            assert_that(automation).listens_to.event(
                "zwave.scene_activated", scene_id="WRONG"
            ).with_callback(automation._my_listen_event_callback)

        with pytest.raises(AssertionError):
            assert_that(automation).listens_to.event(
                "zwave.scene_activated", wrong=3
            ).with_callback(automation._my_listen_event_callback)
...
class TestRegisteredRunDaily:
    def test_success(self, automation: MockAutomation, assert_that):
        ...
        assert_that(automation).registered.run_daily(
            time(hour=3, minute=7), extra_param="ok"
    ...
    def test_failure__wrong_kwargs(
        ...
        with pytest.raises(AssertionError):
            assert_that(automation).registered.run_daily(
                time(hour=3, minute=7), extra_param="WRONG"
            ).with_callback(automation._my_run_daily_callback)

        with pytest.raises(AssertionError):
            assert_that(automation).registered.run_daily(
                time(hour=3, minute=7), wrong="ok"
            ).with_callback(automation._my_run_daily_callback)
...
class TestRegisteredRunMinutely:
    def test_success(self, automation: MockAutomation, assert_that):
        ...
        assert_that(automation).registered.run_minutely(
            time(hour=3, minute=7), extra_param="ok"
...
class TestRegisteredRunAt:
    def test_success(self, automation: MockAutomation, assert_that):
        ...
        assert_that(automation).registered.run_at(
            datetime(2019, 11, 5, 22, 43, 0, 0), extra_param="ok"
    ...
    def test_failure__wrong_time(
        ...
        with pytest.raises(AssertionError):
            assert_that(automation).registered.run_at(
                datetime(2019, 11, 5, 20, 43, 0, 0), extra_param="ok"
            ).with_callback(automation._my_run_at_callback)
    ...
    def test_failure__wrong_kwargs(
        ...
        with pytest.raises(AssertionError):
            assert_that(automation).registered.run_at(
                datetime(2019, 11, 5, 22, 43, 0, 0), extra_param="WRONG"
            ).with_callback(automation._my_run_at_callback)

        with pytest.raises(AssertionError):
            assert_that(automation).registered.run_at(
                datetime(2019, 11, 5, 22, 43, 0, 0), wrong="ok"
            ).with_callback(automation._my_run_minutely_callback)
