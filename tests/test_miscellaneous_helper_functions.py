...
def test_entity_exists_true(
    given_that, with_miscellaneous_helper_functions, hass_functions
):
    given_that.state_of(COVER).is_set_to(
        "closed", {"friendly_name": f"{COVER}", "current_position": 0}
    )
    assert with_miscellaneous_helper_functions.get_entity_exists(COVER) is True
...
def test_entity_exists_false(
    ...
    assert (
        with_miscellaneous_helper_functions.get_entity_exists(
            "not_existent_entity"
        )
        is False
    )
