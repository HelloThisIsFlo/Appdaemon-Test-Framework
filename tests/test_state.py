...
def test_state_was_never_set__raise_error(
...
def test_attribute_was_never_set__raise_error(
    given_that, automation: MockAutomation
):
    given_that.state_of(LIGHT).is_set_to("on")
...
@pytest.mark.only
def test_throw_typeerror_when_attributes_arg_not_passed_via_keyword(
    given_that, automation: MockAutomation
):
    given_that.state_of(LIGHT).is_set_to(
        "on", attributes={"brightness": 11, "color": "blue"}
