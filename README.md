...
1. **Write your first test:**
   ...
   ```
   > ##### Note
   > The following fixtures are **injected** by pytest using the **[`conftest.py`](https://github.com/FlorianKempenich/Appdaemon-Test-Framework/blob/new-features/doc/full_example/conftest.py) file** and the **initialization fixture created at Step 1**:
   > * `living_room`
   > * `given_that`
   > * `assert_that`
   > * `time_travel`
...
*    #### Time
     Note that `given_that.time_is()` must be called prior to initialing any automatons. One way to satisfy this is to make these calls in the `@automation_fixture` setup.
     ```python
     # Command
...
To **move through time** in your automation **use the time_travel fixture**.

It will run the callbacks registered with the following functions of Appdaemon:
* `run_in`
* `run_once`
* `run_at`
* `run_daily`
...
It will also honor calls to `cancel_timer`.

Notes:
* All automation instances **must** be initialized prior to time travel
* Callbacks are executed in the time order they are triggered
* Time time is set for each callback to the time the call was triggered
...
time_travel.fast_forward().to(datetime.time(15, 45))
# Note: trying to flashforward to the past will raise an exception
time_travel.fast_forward().to(datetime.datetime(2020, 10, 19, 4, 35))
...
###### 1. Setup
* **Patch** `hass.Hass` functions and create `HassMock` instance for tracking state.
* **Inject** `hass_mock` in helpers: `given_that`, `assert_that`, `time_travel`
###### 2. Test run
* **Run** the test suite
###### 3. Teardown
...
* #### Pre-initialization setup
  ...
  * `hass_mock` _- For more complex setup steps_
