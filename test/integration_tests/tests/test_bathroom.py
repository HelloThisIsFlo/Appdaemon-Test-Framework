...
def when_new(bathroom):
    class WhenNewWrapper:
        ...
        def no_more_motion_bathroom(self):
            bathroom._no_more_motion_bathroom(None, None, None, None, None)
...
class TestInitialize:
    ...
    def test_start_during_day(
        self, given_that, when_new, assert_that, bathroom, assert_day_mode_started
    ):
        given_that.time_is(time(hour=13))
        bathroom.initialize()
        assert_day_mode_started()

    def test_start_during_evening(
        self, given_that, when_new, assert_that, bathroom, assert_evening_mode_started
    ):
    ...
    def test_callbacks_are_registered(self, bathroom, hass_mocks):
        ...
        run_daily.assert_any_call(
            bathroom._time_triggered, time(hour=DAY_HOUR), hour=DAY_HOUR
        )
        run_daily.assert_any_call(
            bathroom._time_triggered, time(hour=EVENING_HOUR), hour=EVENING_HOUR
        )
...
class TestDuringEvening:
    ...
    class TestEnterBathroom:
        def test_light_turn_on(
            self, given_that, when_new, assert_that, start_evening_mode
        ):
            start_evening_mode()
        ...
        def test__bathroom_playing__unmute(
            self, given_that, when_new, assert_that, start_evening_mode
        ):
            start_evening_mode()
        ...
        def test__entire_flat_playing__unmute(
            self, given_that, when_new, assert_that, start_evening_mode
        ):
            start_evening_mode()
            given_that.state_of(ID["cast_groups"]["entire_flat"]).is_set_to("playing")
            when_new.motion_bathroom()
            assert_bathroom_was_UNmuted(assert_that)

        def test__nothing_playing__do_not_unmute(
            self, given_that, when_new, assert_that, start_evening_mode
        ):
            start_evening_mode()
            when_new.motion_bathroom()
...
class TestsDuringDay:
    ...
    class TestEnterBathroom:
        def test_light_turn_on(self, given_that, when_new, assert_that, start_day_mode):
            start_day_mode()
            when_new.motion_bathroom()
            assert_that(ID["bathroom"]["led_light"]).was.turned_on(color_name=DAY_COLOR)

        def test__bathroom_playing__unmute(
            self, given_that, when_new, assert_that, start_day_mode
        ):
            start_day_mode()
            given_that.state_of(ID["bathroom"]["speaker"]).is_set_to("playing")
            when_new.motion_bathroom()
    ...
    class TestLeaveBathroom:
        def test__no_more_motion__mute_turn_off_light(
            self, given_that, when_new, assert_that, start_day_mode
        ):
            start_day_mode()
            when_new.no_more_motion_bathroom()
        ...
        def test__motion_anywhere_except_bathroom__do_NOT_mute_turn_off_light(
            self, given_that, when_new, assert_that, start_day_mode
        ):
            scenarios = [when_new.motion_kitchen, when_new.motion_living_room]
            for scenario in scenarios:
                start_day_mode()
                given_that.mock_functions_are_cleared()
                scenario()
    ...
    class TestSwitchToNextState:
        def test_click_activate_shower_state(
            self, start_day_mode, when_new, assert_shower_state_started
        ):
            start_day_mode()
            when_new.click_bathroom_button()
            assert_shower_state_started()

        def test_8pm_activate_evening_state(
            self, start_day_mode, when_new, assert_evening_mode_started
        ):
            start_day_mode()
            when_new.time(hour=EVENING_HOUR)
            assert_evening_mode_started()
...
class TestDuringShower:
    ...
    class TestAtStart:
        def test_light_indicator(
            self, given_that, when_new, assert_that, start_shower_mode
        ):
            start_shower_mode()
            assert_that(ID["bathroom"]["led_light"]).was.turned_on(
        ...
        def test_notif_sound(self, assert_that, start_shower_mode):
            ...
            assert_that("xiaomi_aqara/play_ringtone").was.called_with(
                ringtone_id=notif_sound_id,
                ringtone_vol=volume,
                gw_mac=xiaomi_gateway_mac_address,
            )

        def test_mute_all_except_bathroom(
            self, given_that, assert_that, start_shower_mode
        ):
            ...
            for speaker in all_speakers_except_bathroom:
                assert_that("media_player/volume_set").was.called_with(
                    entity_id=speaker, volume_level=FAKE_MUTE_VOLUME
                )

        def test_set_shower_volume_bathroom(
            self, given_that, assert_that, start_shower_mode
        ):
            start_shower_mode()
            assert_that("media_player/volume_set").was.called_with(
    ...
    class TestClickButton:
        def test__activate_after_shower(
            self, given_that, when_new, assert_that, start_shower_mode
        ):
...
class TestDuringAfterShower:
    ...
    class TestAtStart:
        def test_light_indicator(
            self, given_that, when_new, assert_that, start_after_shower_mode
        ):
    ...
    class TestMotionAnywhereExceptBathroom:
        def test_resume_podcast_playback(
            self,
            given_that,
            when_new,
            assert_that,
        ...
        def test_during_day_activate_day_mode(
            self,
            given_that,
            when_new,
            assert_that,
        ...
        def test_during_evening_activate_evening_mode(
            self,
            given_that,
            when_new,
            assert_that,
...
def assert_bathroom_was_muted(assert_that):
    assert_that("media_player/volume_set").was.called_with(
        entity_id=ID["bathroom"]["speaker"], volume_level=FAKE_MUTE_VOLUME
    )
...
def assert_evening_mode_started(when_new, assert_that):
    return lambda: assert_bathroom_light_reacts_to_movement_with_color(
        EVENING_COLOR, when_new, assert_that
    )
