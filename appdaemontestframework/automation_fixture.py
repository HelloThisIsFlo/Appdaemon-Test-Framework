...
def _instantiate_and_initialize_automation(
    ...
    _inject_helpers_and_call_function(
        function, given_that, hass_functions, hass_mocks
    )
...
def _inject_helpers_and_call_function(
    function, given_that, hass_functions, hass_mocks
):
    injectable_fixtures = {
    ...
    def _check_valid(param):
        if param not in injectable_fixtures:
            raise AutomationFixtureError(
                f"'{param}' is not a valid fixture! | The only fixtures "
                "injectable in '@automation_fixture' are: "
                "{list(injectable_fixtures.keys())}"
            )

        if param == "hass_functions":
            warnings.warn(
                """
                Injecting `hass_functions` into automation fixtures is
                deprecated.
                Replace `hass_functions` with `hass_mocks` injections and
                access hass_functions with `hass_mocks.hass_functions`
                """,
                DeprecationWarning,
            )
...
def ensure_automation_is_valid(automation_class):
    ...
    def function_has_arguments_other_than_self(func_name):
        func_parameters = signature(
            getattr(automation_class, func_name)
        ).parameters
    ...
    if not function_exist_in_automation_class("initialize"):
        raise AutomationFixtureError(
            f"'{automation_class.__name__}' has no 'initialize' function! "
            "Make sure you implemented it!"
        )
    if function_has_arguments_other_than_self("initialize"):
        raise AutomationFixtureError(
            f"'{automation_class.__name__}' 'initialize' should have no "
...
class _AutomationFixtureDecoratorWithArgs:
    ...
    def __call__(self, function):
        @pytest.fixture(
            params=self.automation_classes_with_args, ids=self._generate_id
        )
        def automation_fixture_with_initialisation(
            request, given_that, hass_functions, hass_mocks
        ):
...
def automation_fixture(*args):
    ...
    automation. When using parameters, the injected object will be a tuple:
    `(Initialized_Automation, params)`

    # Pre-initialization setup
    All code in the `@automation_fixture` function will be executed before
    initializing the `automation_class`

    3 fixtures are injectable in `@automation_fixture`: 'given_that',
    'hass_mocks' and 'hass_functions'
    ...
    def bathroom():
        pass
    # -> `Bathroom` automation will be initialized and available in tests as
    `bathroom`
    ...
    # -> 2. `Bathroom` automation will be initialized and available in tests
    as `bathroom`
    ...
    if type(args[0]) is not tuple:
        ...
        return _AutomationFixtureDecoratorWithArgs(
