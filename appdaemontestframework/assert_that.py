    ...
    def __init__(
        ...
        message = "\n".join(
            [
                ...
                "",
                (
                    "At least ONE of the following exceptions should not have"
                    "been raised"
                ),
    ...
    def turned_on(self, **service_specific_parameters) -> None:
        ...
        turn_on_helper_not_called = _capture_assert_failure_exception(
            lambda: self.hass_functions["turn_on"].assert_any_call(
                entity_id, **service_specific_parameters
            )
        )
...
class WasNotWrapper(Was):
    ...
    def turned_on(self, **service_specific_parameters) -> None:
        ...
        parameters.
        """
        thing_not_turned_on_with_given_params = (
            _capture_assert_failure_exception(
                lambda: self.was_wrapper.turned_on(
        ...
        class WithCallbackWrapper:
            def with_callback(self, callback):
                ...
                listens_to_wrapper.listen_state.assert_any_call(
                    callback, entity_id, **listen_state_opts
                )
...
class AssertThatWrapper:
    def __init__(self, hass_mocks):
        # Access the `_hass_functions` through private member for now to avoid
        # genearting deprecation warnings while keeping compatibility.
    ...
    def __call__(self, thing_to_check):
        ...
        self._listens_to = ListensToWrapper(
            thing_to_check, self.hass_functions
        )
        self._registered = RegisteredWrapper(
            thing_to_check, self.hass_functions
        )
        return self
