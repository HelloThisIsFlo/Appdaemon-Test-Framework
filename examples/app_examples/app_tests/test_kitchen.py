...
def when_new(kitchen):
    class WhenNewWrapper:
        ...
        def no_more_motion(self):
            kitchen._no_more_motion(None, None, None, None, None)
...
class TestInitialization:
    def test_callbacks_are_registered(self, kitchen, hass_mocks):
        ...
        listen_state.assert_any_call(
            kitchen._no_more_motion, ID["kitchen"]["motion_sensor"], new="off"
        )
...
def assert_water_heater_notif_sent(assert_that):
    def assert_water_heater_sent_wrapper(message):
        assert_that("notify/pushbullet").was.called_with(
            title="Water Heater", message=message, target=PHONE_PUSHBULLET_ID
        )
...
class TestSingleClickOnButton:
    ...
    class TestAfterDelay:
        def test_turn_water_heater_back_on(
        ...
        def test_send_notification(
            self, when_new, time_travel, assert_water_heater_notif_sent
        ):
            when_new.click_button()
            time_travel.fast_forward(SHORT_DELAY).minutes()
...
class TestDoubleClickOnButton:
    ...
    class TestAfterShortDelay:
        def test_DOES_NOT_turn_water_heater_back_on(
            self, when_new, time_travel, assert_that
        ):
            when_new.click_button(type="double")
            time_travel.fast_forward(SHORT_DELAY).minutes()
        ...
        def test_DOES_NOT_send_notification(
            self, when_new, time_travel, assert_water_heater_notif_NOT_sent
        ):
            when_new.click_button(type="double")
            time_travel.fast_forward(SHORT_DELAY).minutes()
...
class TestClickCancellation:
    class TestSingleClick:
        def test_new_click_cancels_previous_one(
            self, when_new, time_travel, assert_that
        ):
            #  T = 0min
            # FF = 0min
    ...
    class TestMixedClicks:
        def test_short_then_long_keep_latest(
            self, when_new, time_travel, assert_that
        ):
            when_new.click_button()
            time_travel.fast_forward(2).minutes()
        ...
        def test_long_then_short_keep_latest(
            self, when_new, time_travel, assert_that
        ):
            when_new.click_button(type="double")
            time_travel.fast_forward(2).minutes()
