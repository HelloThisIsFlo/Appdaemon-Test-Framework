...
class TestDuringEvening:
    ...
    class TestEnterBathroom:
        ...
        def test__entire_flat_playing__unmute(
            ...
            start_evening_mode()
            given_that.state_of(ID["cast_groups"]["entire_flat"]).is_set_to(
                "playing"
            )
            when_new.motion_bathroom()
            assert_bathroom_was_UNmuted(assert_that)
...
class TestDuringShower:
    ...
    class TestAtStart:
        ...
        def test_mute_all_except_bathroom(
            ...
            # Bug with sound bar firmware: Can only increase the volume by 10%
            # at a time to prevent this being a problem, we're not muting it
...
class TestDuringAfterShower:
    ...
    class TestAtStart:
        ...
        def test_turn_off_water_heater(
            self, assert_that, start_after_shower_mode
        ):
            start_after_shower_mode()
...
def assert_bathroom_light_reacts_to_movement_with_color(
    color, when_new, assert_that
):
    when_new.motion_bathroom()
