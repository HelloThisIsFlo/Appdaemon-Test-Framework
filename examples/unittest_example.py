...
# Important:
# This class is equivalent to the setup done in the `conftest.py` on the pytest
# version. Do not forget to inherint from it in all your XXXTestCase classes.
...
class LivingRoomTestCase(AppdaemonTestCase):
    ...
    def test_during_night_light_turn_on(self):
        self.given_that.state_of("sensor.living_room_illumination").is_set_to(
            200
        )  # 200lm == night
